...
export const cargosApi = {
  ...
  atualizar: (id: number, data: CargoPayload) =>
    http.put<Cargo>(`/api/cargos/${id}`, data),
...
export const integrantesApi = {
  ...
  cadastrar: (data: IntegrantePayload) =>
    http.post<Integrante>("/api/integrantes", data),
  atualizar: (id: number, data: IntegrantePayload) =>
    http.put<Integrante>(`/api/integrantes/${id}`, data),
...
export const timesApi = {
  listar: (params?: TimesQueryParams) =>
    http.get<Page<Time>>("/api/times", params),
  ...
  atualizar: (id: number, data: TimePayload) =>
    http.put<Time>(`/api/times/${id}`, data),
...
export const relatoriosApi = {
  ...
  contagemClubes: (inicio?: string, fim?: string) =>
    http.get<Record<string, number>>(
      "/api/contagem-clubes",
      withDateRange(inicio, fim),
    ),

  contagemFuncoes: (inicio?: string, fim?: string) =>
    http.get<Record<string, number>>(
      "/api/contagem-funcoes",
      withDateRange(inicio, fim),
    ),
  ...
  timePorData: async (data: string | Date) => {
    const response = await http.get<{ mensagem: string; time: Time }>(
      "/api/time",
